...
def build(target, options, work_dir, out_dir, env):
	...
	gn = os.path.join(os.getcwd(), 'buildtools', 'win', 'gn.exe')
	ninja = os.path.join(os.getcwd(), 'third_party', 'ninja', 'ninja.exe')
...
def generate_abseil_exports(options, out_dir, env):
	...
	abseil = os.path.join('third_party', 'abseil-cpp')
...
for arch in args.PLATFORMS:
	...
	for lib in args.LIBS:
		...
		for conf in args.CONFIGURATIONS:
			...
			build('v8' if lib == 'shared' else 'v8_monolith', options, work_dir='./', out_dir=out_dir, env=env)
